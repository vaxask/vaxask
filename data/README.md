...
- **`knowledge_base_references.xlsx`** — the 129 sources that make up the knowledge base
  ...
  designated as the authoritative anchor for their category; a blank means the source is a
  regular (non-anchor) member of the corpus. A source may anchor more than one category
  (shown as, e.g., `rank-1 (kat-1 + kat-3)`).
